function opti_vol_edificio_consombra(vec_psVolConSombra, vec_altVolConSombra, vec_pisos, alturaPiso, ps_areaEdif, ps_calles, ps_publico, ps_bruto,
    ...
    for pisos in min_pisos:max_pisos
        ...
        combos_p = generate_stack_vector(pisos, K)
        if pisos == min_pisos
            combos = combos_p
        else
            combos = vcat(combos, combos_p)
        end
    end
    combos = [v for v in combos if v[1] ≥ max_pisos - K]
    ...
    while flag_continue
        ...
        if max_sol >= 0.99 * maxConstruccionSNT || iter_c == length(combos)
            flag_continue = false
        end
    end
