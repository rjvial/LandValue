...
for r in rowSet
    ...
    if conn == conn_LandValue
        fig, ax, ax_mat = polyPlot.plotBaseEdificio3D(fpe, xopt, alturaPiso, ps_predio, ps_volTeorico, matConexionVertices_volTeorico, vecVertices_volTeorico, 
        ...
        fig, ax, ax_mat = polyPlot.plotPolyshape2Din3D(ps_buffer_predio_, 0.0, "gray", 0.15, fig=fig, ax=ax, ax_mat=ax_mat)
