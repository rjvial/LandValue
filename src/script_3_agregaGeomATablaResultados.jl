...
query_resultados_str = """
select combi_predios from tabla_resultados_cabidas
"""
df_resultados = pg_julia.query(conn_LandValue, query_resultados_str)
...
numRows, numCols = size(df_combi_locations)
...
for r = 1:numRows
  ...
  WHERE st_intersects(geom_combi, (SELECT geom FROM combi_locations WHERE id = rrr_ )) 
  """
  query_combi_str = replace(query_combi_str, "rrr_" => string(r))
