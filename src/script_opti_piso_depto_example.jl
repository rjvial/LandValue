...
examples = [
    (width=30.0, height=20.0, rotation=π/6, vec_sup_deptos=[50.0, 75.0, 100.0], vec_num_deptos=[2, 3, 2], ancho_pasillo=2.0, vec_sup_terraza=[50.0/6, 75.0/6, 100.0/6]),
    (width=25.0, height=25.0, rotation=π/4, vec_sup_deptos=[60.0, 80.0], vec_num_deptos=[3, 3], ancho_pasillo=2.5, vec_sup_terraza=[60.0/6, 80.0/6]),
    (width=35.0, height=18.0, rotation=0.0, vec_sup_deptos=[45.0, 65.0, 85.0, 110.0], vec_num_deptos=[2, 2, 2, 1], ancho_pasillo=2.0, vec_sup_terraza=[45.0/6, 65.0/6, 85.0/6, 110.0/6]),
    (width=28.0, height=22.0, rotation=-π/8, vec_sup_deptos=[55.0, 90.0], vec_num_deptos=[4, 2], ancho_pasillo=2.2, vec_sup_terraza=[55.0/6, 90.0/6]),
    (width=32.0, height=16.0, rotation=π/3, vec_sup_deptos=[40.0, 70.0, 100.0], vec_num_deptos=[3, 2, 2], ancho_pasillo=1.8, vec_sup_terraza=[40.0/6, 70.0/6, 100.0/6])
]
...
for (idx, example) in enumerate(examples)
    ...
    println("  Total required area: $(sum(vec_sup_deptos .* vec_num_deptos))m²")
    println("  Core width: $(example.ancho_pasillo)m")
    println("  Terrace areas: $(vec_sup_terraza)m²")

    results = opti_floor_plan(ps_planta, vec_sup_deptos, vec_num_deptos, ancho_pasillo=example.ancho_pasillo, vec_sup_terraza=vec_sup_terraza)
    ...
    if results["feasible"]
        ...
        println("  Unused area: $(round(results["holgura_total"], digits=2))m²")
