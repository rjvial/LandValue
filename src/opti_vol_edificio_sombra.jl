function opti_vol_edificio_sombra(vec_psVolConSombra, vec_altVolConSombra, vec_pisos, alturaPiso, ps_areaEdif, ps_calles, ps_publico, ps_bruto,
    ...
    while iter < max_iter && min(delta_p, delta_o, delta_s) < 0
        ...
        ps_stack, np_stack = opti_vol_edificio(vec_psVolConSombra, vec_altVolConSombra,
            vec_pisos, alturaPiso, max_ocupacion_suelo, maxConstruccionSNT, K; ancho_crujia_edificio = ancho_crujia_edificio, flag_reverse = false
        )

        # Compute shadows cumulatively
        ps_sombraEdif_p, ps_sombraEdif_o, ps_sombraEdif_s = generaSombraEdificio(
            ps_stack,
            cumsum(np_stack).*alturaPiso,
            ps_publico,
            ps_calles
        )
        ...
        delta_p = area_act_p>0 ? areaSombra_p/area_act_p -1 : 1
        delta_o = area_act_o>0 ? areaSombra_o/area_act_o -1 : 1
        delta_s = area_act_s>0 ? areaSombra_s/area_act_s -1 : 1
