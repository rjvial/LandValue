...
function plotBaseEdificio3D(ps_predio, vec_ps, vec_np, max_sol, vec_psVolteor, vec_altVolteor, vec_psVolConSombra, vec_altVolConSombra, ps_sombraEdif_p, ps_sombraEdif_o, ps_sombraEdif_s, ps_sombraVolTeorico_p, ps_sombraVolTeorico_o, ps_sombraVolTeorico_s)
    fig = nothing; ax = nothing; ax_mat = nothing
    alturaPiso = 3.0

    # Plot predio
    if ps_predio !== nothing
        fig, ax, ax_mat = polyPlot.plotPolyshape2Din3D(ps_predio, 0.0, "green", 0.3, fig=fig, ax=ax, ax_mat=ax_mat)
    end

    # Plot building floors
    color = "teal"
    fig, ax, ax_mat = plot_building_floors(vec_ps, vec_np, alturaPiso, color, 1.0, fig, ax, ax_mat)
    
    # Plot max_sol if available
    if max_sol !== nothing && isa(max_sol, Union{PolyShape, LineShape, PointShape})
        fig, ax, ax_mat = polyPlot.plotPolyshape2Din3D(max_sol, 0, "blue", 0.5, fig=fig, ax=ax, ax_mat=ax_mat)
    end

    # Plot theoretical volume outlines
    fig, ax, ax_mat = polyPlot.plotPolyshape2DVecin3D(vec_psVolteor, vec_altVolteor, "red", 0.001, fig=fig, ax=ax, ax_mat=ax_mat, edge_color="red", line_width=0.05)
    
    # Plot volume with shadows outlines
    fig, ax, ax_mat = polyPlot.plotPolyshape2DVecin3D(vec_psVolConSombra, vec_altVolConSombra, "gray", 0.01, fig=fig, ax=ax, ax_mat=ax_mat, edge_color="gray", line_width=0.1)

    # Plot building shadows
    for ps_sombra in [ps_sombraEdif_p, ps_sombraEdif_o, ps_sombraEdif_s]
        if ps_sombra !== nothing
            fig, ax, ax_mat = polyPlot.plotPolyshape2Din3D(ps_sombra, 0, "red", 0.25, fig=fig, ax=ax, ax_mat=ax_mat)
        end
    end

    # Plot theoretical shadows
    for ps_sombra in [ps_sombraVolTeorico_p, ps_sombraVolTeorico_o, ps_sombraVolTeorico_s]
        if ps_sombra !== nothing
            fig, ax, ax_mat = polyPlot.plotPolyshape2Din3D(ps_sombra, 0, "gold", 0.3, fig=fig, ax=ax, ax_mat=ax_mat)
        end
    end
