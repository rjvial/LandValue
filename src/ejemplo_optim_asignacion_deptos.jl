...
mat_reg_aux = [vec_area_interior[i] + vec_area_pasillo[j] for i in set_i, j in set_w]
mat_reg_h = [mat_reg_aux[i,j] / vec_w[j] for i in set_i, j in set_w]

mat_corner_h = [vec_area_interior[i] / vec_w[j] for i in set_i, j in set_w]
mat_double_corner_h = [vec_area_interior[i] / vec_w[j] for i in set_i, j in set_w]
...
mat_exposicion_corner = [vec_w[i] + mat_corner_h[i,j] for i in eachindex(vec_area_interior), j in eachindex(set_w)]
mat_exposicion_double_corner = [vec_w[i] + 2*mat_double_corner_h[i,j] for i in eachindex(vec_area_interior), j in eachindex(set_w)]
